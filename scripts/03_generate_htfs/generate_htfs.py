...
parser.add_argument('protein_filename', type=str, help='protein file')
...
# Generate htfs
solvent_delivery = PointMutationExecutor(args.protein_filename,
                        '1',
                        args.resid,
                        args.mutant_aa,
...
with open(os.path.join(args.outdir, f"{directory_number}_{phase}.pickle"), "wb") as f:
    pickle.dump(apo_rest_htf, f)

if args.ligand_input:
    complex_rest_htf = solvent_delivery.get_complex_rest_htf()
    with open(os.path.join(args.outdir, f"{directory_number}_complex.pickle"), "wb") as f:
        pickle.dump(complex_rest_htf, f)
...
# Save pdbs
app.PDBFile.writeFile(apo_rest_htf._topology_proposal.old_topology, apo_rest_htf.old_positions(apo_rest_htf.hybrid_positions), open(os.path.join(args.outdir, f"{directory_number}_{phase}_old.pdb"), "w"), keepIds=True)
app.PDBFile.writeFile(apo_rest_htf._topology_proposal.new_topology, apo_rest_htf.new_positions(apo_rest_htf.hybrid_positions), open(os.path.join(args.outdir, f"{directory_number}_{phase}_new.pdb"), "w"), keepIds=True)

if args.ligand_input:
    app.PDBFile.writeFile(complex_rest_htf._topology_proposal.old_topology, complex_rest_htf.old_positions(complex_rest_htf.hybrid_positions), open(os.path.join(args.outdir, f"{directory_number}_complex_old.pdb"), "w"), keepIds=True)
    app.PDBFile.writeFile(complex_rest_htf._topology_proposal.new_topology, complex_rest_htf.new_positions(complex_rest_htf.hybrid_positions), open(os.path.join(args.outdir, f"{directory_number}_complex_new.pdb"), "w"), keepIds=True)
